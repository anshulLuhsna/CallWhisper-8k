...
- Done: Created `datasets/manifests/gramvaani_dev_10.csv`, ran Whisper `tiny` and `base`, and saved baseline result files.
- Blocked: None.
- Next action: Run Whisper `small` if local runtime is acceptable, otherwise move larger runs to Colab/GPU.

## Scoreboard

- Day-2 WER number shipped: yes
- Baseline table shipped: no
