...
The 500-speaker profile is now complete. ARTPARK's pooled channel penalty was `+0.0049`; Adalat's was `+0.0280`. Their penalty difference was `+0.0232`, with a paired speaker bootstrap 95% CI of `[+0.0166, +0.0300]`. GSM-FR produced the largest penalty and robustness gap. See `results/vaani_paired_model_full_v1.md`.

Provenance changes the claim boundary: ARTPARK's model card says Vaani was part of its fine-tuning mixture, and Vaani Benchmark V1.0 is derived from Vaani. Exact utterance overlap is not documented. Treat ARTPARK as an in-domain upper baseline, preserve the paired degradation finding, and require external held-out replication before presenting its absolute WER advantage as a clean generalization result.

The LAHAJA replication gate is now complete on all 132 speakers. ARTPARK's pooled penalty was `+0.0058`; Adalat's was `+0.0350`; and the Adalat-minus-ARTPARK penalty gap was `+0.0292` with 95% CI `[+0.0137, +0.0459]`. This externally replicates the compact model's higher telephone sensitivity. Adalat remained about `1.88x` faster, while absolute original and pooled model-gap intervals included zero. See `results/lahaja_paired_external_v1.md`.

The next research phase is therefore mitigation, not more evidence-free model shopping: recover the raw LAHAJA report bundle, audit current Transformers attention-mask behavior, freeze subgroup analyses, curate benchmark-disjoint training/internal-validation manifests, and train Adalat Whisper-small with balanced channel augmentation plus clean replay.
