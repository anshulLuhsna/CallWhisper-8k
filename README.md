...
The 500-speaker v1 construction run passed every automated integrity check, but manual listening caught an important semantic issue before model evaluation: codec-only G.711/GSM rows sounded much closer to the original than the explicit 300-3400 Hz condition. See [results/vaani_paired_pilot_v1_validation.md](results/vaani_paired_pilot_v1_validation.md). The corrected v2 artifact is now frozen: 500 speakers, 2,500 complete paired files, zero missing or duration violations, and a passed listening check. See [results/vaani_paired_pilot_v2_validation.md](results/vaani_paired_pilot_v2_validation.md). The next gate is a tested multi-reference scorer and same-pipeline ARTPARK-versus-Adalat smoke inference.

That gate is implemented in [notebook 12](notebooks/12_vaani_paired_artpark_adalat_smoke_colab.ipynb). Its default smoke profile runs pinned ARTPARK medium and Adalat Whisper-small on the same 10 speakers under all five channels, checkpoints every prediction to Drive, and reports alignment-based multi-reference WER. The 10-speaker output validates the pipeline; it is not a publishable benchmark result.
