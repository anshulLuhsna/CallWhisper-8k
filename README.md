...
The 500-speaker v1 construction run passed every automated integrity check, but manual listening caught an important semantic issue before model evaluation: codec-only G.711/GSM rows sounded much closer to the original than the explicit 300-3400 Hz condition. See [results/vaani_paired_pilot_v1_validation.md](results/vaani_paired_pilot_v1_validation.md). The canonical [notebook 11](notebooks/11_vaani_paired_telephony_benchmark_colab.ipynb) now stacks the telephone bandlimit before every codec. For the already-completed v1 artifacts, run [notebook 11b](notebooks/11b_vaani_stacked_codec_repair_colab.ipynb) to create v2 without redownloading Vaani. The existing [notebook 10](notebooks/10_gv_train_100h_inventory_colab.ipynb) remains the later training-data inventory step.
