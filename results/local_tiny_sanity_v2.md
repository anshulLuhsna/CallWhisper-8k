...
This is a local development sanity check for the expanded GramVaani 100-file slice. It is not a headline benchmark result.

## Purpose

The goal was to verify that the new `gramvaani_dev_100.csv` manifest can run end-to-end through the existing OpenAI Whisper evaluation runner and produce WER/CER outputs.
...
- This result is mainly a pipeline check. It should not be compared as a final model result against GPU runs unless the same manifest, language mode, seed, normalization, and decoding settings are documented.
